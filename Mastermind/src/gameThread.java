...
public class gameThread extends Thread {
    ...
    private static String processGuess(String guess, String code) {
        ...
        if (response.length() < 4) { // Not all positions filled with 'P'
            for (int i = 0; i < 4; i++) {
                ...
                //check if characters are not equal to space, otherwise it will also count as matching
                if (codeChar != ' ') {
    ...
    public void run() {
        try (DataInputStream in = new DataInputStream(client.getInputStream());
            ...
            System.out.println("Game string to win is: " + codeBuilder); //print the game code
...
//                gameString = processGuess(gameString, "BGWG");
